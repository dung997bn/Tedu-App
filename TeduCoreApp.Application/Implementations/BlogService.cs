...
namespace TeduCoreApp.Application.Implementations
{
    public class BlogService : IBlogService
    {
        ...
        public BlogService(IBlogRepository blogRepository, ITagRepository tagRepository,
            IBlogTagRepository blogTagRepository, IUnitOfWork unitOfWork, IMapper mapper)
        {
        ...
        public Task<List<BlogViewModel>> GetLastest(int top)
        {
        ...
        {
            return _mapper
                .ProjectTo<BlogViewModel>(_blogRepository.FindAll(x => x.Status == Status.Active && x.HotFlag == true)
        ...
        public List<BlogViewModel> Search(string keyword, int page, int pageSize, string sort, out int totalRow)
        {
            ...
            return _mapper
                .ProjectTo<BlogViewModel>(query.Skip((page - 1) * pageSize)
                .Take(pageSize))
                .ToList();
        }
        ...
        {
            return _mapper
            .ProjectTo<BlogViewModel>(_blogRepository.FindAll(x => x.Status == Status.Active && x.Id != id)
            .OrderByDescending(x => x.DateCreated)
            .Take(top))
            .ToList();
        }
        ...
        {
            return _mapper
                .ProjectTo<TagViewModel>(_blogTagRepository.FindAll(x => x.BlogId == id, c => c.Tag)
                .Select(y => y.Tag))
                .ToList();
        }
